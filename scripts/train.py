...
def main(args):
    ...
    print("Loaded", X.shape, y.shape, "classes:", classes)

    idx = np.arange(len(y))
    np.random.shuffle(idx)
    X = X[idx]; y = y[idx]
    # Kiểu dữ liệu đã là float32 từ bước tạo spectrogram
    y = y.astype('int32')
    ...
    input_shape = X.shape[1:] # Sẽ là (128, 128, 3)
    model = build_model(input_shape, num_classes, weights="imagenet")
    
    model.compile(
        optimizer=keras.optimizers.Adam(learning_rate=5e-5), # Dùng learning rate thấp
